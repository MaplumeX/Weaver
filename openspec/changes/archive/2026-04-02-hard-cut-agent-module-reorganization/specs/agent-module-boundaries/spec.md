...
**Reason**: 本次变更目标是 hard cut 完整迁移，不再保留任何长期 compat 目录或适配层语义。
**Migration**: 将所有调用方、测试 patch 点和 facade/runtime 导入直接迁移到新的 owning modules；删除 `agent.compat.*`，不要以新 shim 替代旧 shim。
