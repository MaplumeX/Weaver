...
**Reason**: Deep Research 的正式控制平面不再由 planner 持有，而是改由 `supervisor` 接收批准后的 scope 契约。
**Migration**: 使用 `supervisor` 读取已批准 scope 并产出 branch 任务；不要再假设存在独立 planner 作为批准范围的唯一消费者。
