...
from agent.runtime.deep.researcher_runtime import assess
...
from agent.runtime.deep.researcher_runtime.shared import canonical_url, dedupe_strings
from agent.runtime.deep.researcher_runtime.state import BranchResearchState
        ...
        *,
        ...
        while state.round_index < state.max_rounds:
            ...
            state.query_rounds.append(
                BranchQueryRoundArtifact(
                    id=support._new_id("branch_round"),
                    task_id=task.id,
                    section_id=task.section_id,
                    branch_id=task.branch_id,
            ...
            state.research_decisions.append(
                BranchDecisionArtifact(
                    id=support._new_id("branch_decision"),
                    task_id=task.id,
                    section_id=task.section_id,
                    branch_id=task.branch_id,
                    round_index=state.round_index,
        ...
        coverage_artifact = BranchCoverageArtifact(
            id=support._new_id("branch_coverage"),
            task_id=task.id,
            section_id=task.section_id,
            branch_id=task.branch_id,
            criteria=list(latest_coverage.criteria),
            covered_count=latest_coverage.covered_count,
            partial_count=latest_coverage.partial_count,
            missing_count=latest_coverage.missing_count,
            missing_topics=list(latest_coverage.missing_topics),
            coverage_ready=latest_coverage.coverage_ready,
        ).to_dict()
        quality_artifact = BranchQualityArtifact(
            id=support._new_id("branch_quality"),
            task_id=task.id,
            section_id=task.section_id,
            branch_id=task.branch_id,
            authority_score=latest_quality.authority_score,
            freshness_score=latest_quality.freshness_score,
            source_diversity_score=latest_quality.source_diversity_score,
            evidence_density_score=latest_quality.evidence_density_score,
            objective_alignment_score=latest_quality.objective_alignment_score,
            quality_ready=latest_quality.quality_ready,
            notes=latest_quality.notes,
        ).to_dict()
        contradiction_artifact = BranchContradictionArtifact(
            id=support._new_id("branch_contradiction"),
            task_id=task.id,
            section_id=task.section_id,
            branch_id=task.branch_id,
            has_material_conflict=latest_contradiction.has_material_conflict,
            conflict_count=latest_contradiction.conflict_count,
            conflict_source_urls=list(latest_contradiction.conflict_source_urls),
            conflict_notes=list(latest_contradiction.conflict_notes),
            needs_counterevidence_query=latest_contradiction.needs_counterevidence_query,
        ).to_dict()
        grounding_artifact = BranchGroundingArtifact(
            id=support._new_id("branch_grounding"),
            task_id=task.id,
            section_id=task.section_id,
            branch_id=task.branch_id,
            claims=list(latest_grounding.claims),
            total_claim_count=latest_grounding.total_claim_count,
            grounded_claim_count=latest_grounding.grounded_claim_count,
            primary_grounding_ratio=latest_grounding.primary_grounding_ratio,
            secondary_grounding_ratio=latest_grounding.secondary_grounding_ratio,
            grounding_ready=latest_grounding.grounding_ready,
        ).to_dict()
