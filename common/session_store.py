...
class SessionStore:
    ...
    async def create_session(
        ...
        *,
        ...
        await self._execute(
            (
                "INSERT INTO sessions (thread_id, user_id, title, route, status) "
                "VALUES (%s, %s, %s, %s, %s)"
            ),
        ...
        **payload: Any,
        ...
        await self._execute(
            (
                ...
                ") VALUES ("
                "%s, %s, COALESCE((SELECT MAX(seq) + 1 FROM session_messages "
                "WHERE thread_id = %s), 1), "
                "%s, %s, %s, %s, %s, %s, %s, %s, %s"
                ")"
            ),
    ...
    async def get_snapshot(self, thread_id: str) -> dict[str, Any]:
        session = await self._fetchrow(
            "SELECT thread_id, user_id, title, summary, context_snapshot, status, route, "
            "is_pinned, tags, created_at, updated_at "
        ...
        messages = await self._fetchall(
            "SELECT id, seq, role, content, attachments, sources, tool_invocations, "
            "process_events, metrics, created_at, completed_at "
    ...
    async def list_messages_after_seq(
        ...
        *,
        ...
        rows = await self._fetchall(
            "SELECT id, seq, role, content, attachments, sources, tool_invocations, "
            "process_events, metrics, created_at, completed_at "
            "FROM session_messages WHERE thread_id = %s AND seq > %s ORDER BY seq ASC LIMIT %s",
    ...
    async def list_sessions(
        ...
        *,
        ...
        if owner:
            rows = await self._fetchall(
                "SELECT thread_id, user_id, title, summary, status, route, is_pinned, "
    ...
    async def update_session_metadata(
        ...
        values: list[Any] = []
        fields = ("title", "summary", "context_snapshot", "is_pinned", "tags", "route", "status")
        for field in fields:
            if field not in updates:
                continue
            assignments.append(f"{field} = %s")
            value = updates[field]
            if field in {"tags", "context_snapshot"}:
                value = Jsonb(value)
            values.append(value)
